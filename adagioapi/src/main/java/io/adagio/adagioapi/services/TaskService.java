...
public class TaskService {
	...
	public boolean validTaskTime(Task task, Long userId, TaskRepository taskRepository) {
		List<Task> tasksBack = taskRepository.findByDate_Time_EndAndUser_IdNative(task.getDateTimeEnd().toLocalDate(),
				userId);

		for (Task t : tasksBack) {
			if (task.getDateTimeStart().isBefore(t.getDateTimeEnd())
					&& task.getDateTimeEnd().isAfter(t.getDateTimeStart())) {
	...
	public boolean validTaskTime(Task task, Long userId, TaskRepository taskRepository, Long taskId) {
		...
		for (Task t : tasksBack) {
			if ((task.getDateTimeStart().isBefore(t.getDateTimeEnd())
					&& task.getDateTimeEnd().isAfter(t.getDateTimeStart())) && taskId != t.getId()) {
				return false;
			}
		}
	...
	public ArrayList<Notification> notificationPattern(List<Notification> notifications) {
		boolean notificationPopup = false, notificationSound = false, notificationEmail = false;
		Category popup = Category.POPUP, sound = Category.SOUND, email = Category.EMAIL;
